...
const COMPACT_BOOKING_HEIGHT = 36
...
export const BookingTimeline = ({
  ...
  const selectedBookingRef = useRef<HTMLElement | null>(null)
  ...
  useEffect(() => {
    selectedBookingRef.current?.scrollIntoView({ block: "center", behavior: "smooth" })
  ...
  return (
    <div
      ref={timelineRef}
      className={cn(
        blocked ? "relative opacity-45" : "relative cursor-default touch-none select-none"
      )}
    ...
    >
      ...
      {bookings.map((booking) => {
        ...
        const compact = height < COMPACT_BOOKING_HEIGHT
        ...
        return (
          <article
            ref={selected ? selectedBookingRef : undefined}
            data-booking
            draggable={false}
            key={booking.id}
            className={cn(
              "group/booking absolute left-12 right-1 z-10 overflow-hidden border border-primary/60 px-2 py-1 shadow-sm outline outline-1 -outline-offset-1 outline-transparent hover:z-20 hover:outline-2 hover:outline-primary focus-within:z-20 focus-within:outline-2 focus-within:outline-primary",
          ...
          >
            ...
            <div
              className={cn(
                "pointer-events-none relative z-10 flex items-start gap-1",
                fifteenMinuteLayout && "h-full items-center"
              )}
            >
              <div
                className={cn("min-w-0 flex-1", fifteenMinuteLayout && "flex items-center gap-1")}
              >
                <p
                  className={cn(
                    "truncate text-[11px] font-semibold",
                    fifteenMinuteLayout && "min-w-0 flex-1 leading-none"
                  )}
                >
                  {localize(booking.title, i18n.language)}
                </p>
                {fifteenMinuteLayout && (
                  <span className="shrink-0 text-[8px] leading-none opacity-75">
                    <BookingTimeRange start={displayStart} end={displayEnd} />
                  </span>
                )}
                {fifteenMinuteLayout && (
                  <span className="max-w-[45%] shrink-0 truncate text-[8px] leading-none opacity-75">
              ...
              {canDrag && (
                <Button
                  ...
                  variant="ghost"
                  className={cn(
                    "pointer-events-auto relative z-30 transition-colors duration-200 hover:bg-transparent hover:text-primary",
                    compact && "size-4"
                  )}
              ...
              {own && new Date(booking.endAt) >= now && (
                <Button
                  data-modal-opener="timeline-cancel-booking"
                  size="icon-xs"
                  variant="ghost"
                  className={cn(
                    "pointer-events-auto relative z-30 transition-colors duration-200 hover:bg-transparent hover:text-destructive",
                    compact && "size-4"
                  )}
