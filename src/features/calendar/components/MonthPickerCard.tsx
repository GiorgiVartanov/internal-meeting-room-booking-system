...
export const MonthPickerCard = ({
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden="true"
        className="grid min-h-0 flex-1 grid-cols-7 grid-rows-7 gap-y-0 text-center text-[10px] sm:text-[9px] lg:text-[10px] xl:text-[11px]"
      >
        {weekdays.map((day) => (
          <span
            key={day.toISOString()}
        ...
        {days.map((day) => (
          <span
            key={day.toISOString()}
            className={cn(
              "flex size-5 min-h-0 items-center justify-center self-center justify-self-center text-[10px] leading-none text-foreground sm:size-4 sm:text-[9px] sm:leading-4 lg:text-[10px] lg:leading-4 xl:text-[11px]",
